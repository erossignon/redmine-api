...
function retrieve_cookie_and_perform_action(configuration, callback) {

    var fs = require('fs');

    if (!fs.existsSync(configuration.cache_folder)) {
        fs.mkdirSync(configuration.cache_folder);
    }

    if (configuration.cookie === null) {
        get_cookie(configuration.url, configuration.username, configuration.password, function (err, cookie) {
...
function perform_http_get_transaction(server, command, callback) {
    ...
    assert(server.cookie, "no version");
    assert(_.isFunction(callback));
 ...
 */
function _fetchVersions(server, project, callback) {

    "use strict";

    var filename = server.cache_folder + project + "_versions" + ".json";

    perform_http_get_transaction(server, "/projects/" + project + "/versions.json", function (err, txt) {
...
function _fetchProjectName(server, callback) {

    assert(server instanceof RedmineWebServer)
    assert(_.isFunction(callback));

    perform_http_get_transaction(server, "projects.json", function (err, txt) {
        var result = JSON.parse(txt);
        callback(err, result.projects);
    });
}
/**
...
function _fetchProjectIssues(server, project, callback, progress_callback) {
    ...
    function internal_fetch(page) {
        ...
        var filename = path.join(server.cache_folder, project + "_page" + page + ".json");
        ...
        perform_http_get_transaction(server, "projects/" + project + "/issues.json?limit=" + limit + "&page=" + page + "&status_id=*", function (err, txt) {
            //xxconsole.warn(" Status Code  = ", res.statusCode);
            if (!err) {
                txt = fixUnicode(txt);
                var o = JSON.parse(txt);
                fs.writeFileSync(filename, JSON.stringify(o, null, " "));
                ...
                o.issues.forEach(function (issue) {
                    list_of_items.push({ id: issue.id, updated_on: issue.updated_on});
                });
...
RedmineWebServer.prototype.getCookie = function (callback) {
    ...
    var self = this;
    self.cookie = null;
    retrieve_cookie_and_perform_action(self, callback);
};
...
RedmineWebServer.prototype.fetchProjectName = function (callback) {
    var self = this;
    retrieve_cookie_and_perform_action(self, function (err) {
        _fetchProjectName(self, function (err, projects) {
            if (!err) {
                self._projects = projects;
                self._projects_names = projects.map(function (p) {
                    return p.name;
                });
                self._projects_id = projects.map(function (p) {
                    return p.id;
                });
...
RedmineWebServer.prototype.fetchVersions = function (project, callback) {
    ...
    var self = this;
    retrieve_cookie_and_perform_action(self, function (err) {
        _fetchVersions(self, project, callback);
    });
};
...
RedmineWebServer.prototype.fetchProjectIssues = function (project, callback) {
    ...
    retrieve_cookie_and_perform_action(self, function (err) {
        ...
        _fetchProjectIssues(self, proj.identifier,function(err,list_of_issues){
    ...
    fs.stat(filename, function (err, stat) {
        ...
        if (!err) {
            ...
            self._read_ticket_from_cache(ticketid, function (err,ticket) {
                ...
                var issue_light = self._issues_light ? self._issues_light.find(function(e){ return e.id === ticketid}): null;
                issue_light = (issue_light && issue_light.length===1) ? issue_light[0]:null;

                // compare the date
                if (issue_light && (ticket.updated_on < issue_light.updated_on) ) {
